...
export const validateResponse = async (
    response: Response,
    dataSchemaName: string | undefined
) => {
    const ajv = new Ajv();
    ...
    const dataSchema = dataSchemaName
        ? await getSchema(dataSchemaName)
        : { type: "null" };

    // Append grout-data $id to the required data schema so that the $ref in response schema targeting that $id is
    // found.
