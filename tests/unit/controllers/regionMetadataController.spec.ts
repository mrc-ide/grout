...
vi.mock("fs");
const mockJsonResponseSuccess = vi.spyOn(jsonResponse, "jsonResponseSuccess").mockImplementation(() => {});
...
describe("RegionMetadataController", () => {
    test("returns level 0 metadata", () => {
        writeTestFile(`/testRoot/region_metadata/${dataset}/0`, "region_metadata_0.json", testMetadata);
        ...
        expect(mockJsonResponseSuccess).toHaveBeenCalledWith(testMetadata, mockRes);
    });

    test("returns country metadata",  () => {
        writeTestFile(`/testRoot/region_metadata/${dataset}/2`, "region_metadata_FRA_2.json", testMetadata);
